  ...
  methods: {
    ...
    async modificarFicha() {
      ...
      if (this.$v.fichaIngreso.$invalid) {
        ...
        this.fichaIngreso.historialcontenido.push({
    ...
    generarPDF(){
        ...
        doc.autoTable({
          ...
          body: [
            [""],[""],[""],[""],
            [{ content: 'FICHA EDUCATIVA INGRESO',styles: { halign: 'center', fontStyle:'bold', fontSize: 12 } }],
            [{ content: 'I. CENTRO DE ATENCION RESIDENCIAL: ' + "CAR DOMI", styles: { fontStyle:'bold', fontSize: 11 } }],
            [{ content: '   Distrito: ' + "ATE"+ '          Provincia: ' + "LIMA"+'          Región: ' + "LIMA",styles: { fontStyle:'normal', fontSize: 11 } }],
            [{ content: 'II. DATOS PERSONALES: ', styles: { fontStyle:'bold', fontSize: 11 } }],
            [{ content: '1. Apellidos y Nombres: '+ this.residenteDocumento.apellido +" "+this.residenteDocumento.nombre, styles: { fontStyle:'normal', fontSize: 11, cellPadding: {left: 3} } }],
            [{ content: '2. Lugar y Fecha de Nacimiento: ' + this.residenteDocumento.lugarnacimiento + " " + this.convertirFechas(this.residenteDocumento.fechanacimiento), styles: { fontStyle:'normal', fontSize: 11, cellPadding: {left: 3}} }],
            [{ content: '3. DNI: ' + this.residenteDocumento.numerodocumento+ '          Edad: '+this.residenteEdad() +' años' +'          Sexo: ' + this.residenteDocumento.sexo ,styles: { fontStyle:'normal', fontSize: 11, cellPadding: {left: 3} } }],
            [{ content: '4. Fecha de Ingreso al CAR: '+ this.convertirFechas(this.residenteDocumento.fechaingreso),styles: { fontStyle:'normal', fontSize: 11, cellPadding: {left: 3} } }],
            [{ content: '5. Educador Responsable: '+ this.usua.datos.nombre + " " + this.usua.datos.apellido,styles: { fontStyle:'normal', fontSize: 11, cellPadding: {left: 3} } }],
            [{ content: '6. Fecha de Evaluación: '+ new Date().toLocaleDateString(),styles: { fontStyle:'normal', fontSize: 11, cellPadding: {left: 3} } }],
            [{ content: '--------------------------------------------------------------------------------------------------------------------------',styles: { fontStyle:'bold', fontSize: 11 } }],
            [{ content: 'III. DATOS DE LA INSTITUCIÓN EDUCATIVA DE PROCEDENCIA:', styles: { fontStyle:'bold', fontSize: 11 } }],
            [{ content: '1. IE: '+ this.fichaIngreso.contenido.ieprocedencia.nombre, styles: { fontStyle:'normal', fontSize: 11, cellPadding: {left: 7} } }],
            [{ content: this.tipoInstitucion(), styles: { fontStyle:'normal', fontSize: 11, cellPadding: {left: 11} } }],
            [{ content: '2. Modalidad: ' + this.fichaIngreso.contenido.ieprocedencia.modalidad+ '          Nivel/Ciclo: ' + this.fichaIngreso.contenido.ieprocedencia.nivel+'          Grado: ' + this.fichaIngreso.contenido.ieprocedencia.grado,styles: { fontStyle:'normal', fontSize: 11, cellPadding: {left: 7} } }],
            [{ content: 'Observación: ' + this.fichaIngreso.contenido.ieprocedencia.observacion,styles: { fontStyle:'normal', fontSize: 11, cellPadding: {left: 11} } }],
            [{ content: '3. Dirección de la I.E.: '+ this.fichaIngreso.contenido.ieprocedencia.direccion, styles: { fontStyle:'normal', fontSize: 11, cellPadding: {left: 7} } }],
            //[{ content: 'Distrito: '+ 'Dsitrito' + '           '+'Provincia: '+ 'Provincia x' + '           ' +'Región: '+ 'Huánuco', styles: { fontStyle:'normal', fontSize: 11, cellPadding: {left: 11} } }],
            [{ content: 'Teléfono: '+ this.fichaIngreso.contenido.ieprocedencia.telefono + '                        '+'Correo Electrónico: '+ this.fichaIngreso.contenido.ieprocedencia.correo , styles: { fontStyle:'normal', fontSize: 11, cellPadding: {left: 11} } }],
            [{ content: '4. Documentos Escolares: '+ this.documentosEscolaresPdf(), styles: { fontStyle:'normal', fontSize: 11, cellPadding: {left: 7}  } }],
            [{ content: '5. SITUACION ESCOLAR: '+ this.fichaIngreso.contenido.ieprocedencia.situacionEscolar, styles: { fontStyle:'normal', fontSize: 11, cellPadding: {left: 7}  } }],
            [{ content: 'IV. DATOS DEL PAP RESPONSABLE DE TURNO EN EL CAR:', styles: { fontStyle:'bold', fontSize: 11 } }],
            [{ content: '1. Apellidos y Nombres: '+ this.usua.datos.nombre + " " + this.usua.datos.apellido, styles: { fontStyle:'normal', fontSize: 11, cellPadding: {left: 7} } }],
            [{ content: 'DNI: '+ this.usua.datos.numerodocumento + '           ' +'Correo Electrónico: '+ this.usua.datos.email, styles: { fontStyle:'normal', fontSize: 11 , cellPadding: {left: 11}} }],
            [{ content: 'V. OBSERVACIONES:', styles: { fontStyle:'bold', fontSize: 11 } }],
            [{ content: this.observacionesPdf(),styles: { fontStyle:'normal', fontSize: 11, cellPadding: {left: 7} } }],
          ],
      ...
      tipoInstitucion(){
        if(this.fichaIngreso.contenido.ieprocedencia.tipo == 'PRIVADA'){
          return 'Estatal: '+ '' + '           '+'Privado: '+'X'
        }else{
          return 'Estatal: '+ 'X' + '           '+'Privado: '+''
        }
      },
      observacionesPdf(){
        var observaciones = "";
        this.fichaIngreso.contenido.observaciones.forEach(element => 
          observaciones += "- "+ element + "\n"
        );
        return observaciones
      },
      documentosEscolaresPdf(){
        var documentosEscolares = "";
        if(this.fichaIngreso.contenido.ieprocedencia.documentosEscolares.length == 0){
          return 'No tiene documentación';
        }else{
          documentosEscolares += "\n";
          this.fichaIngreso.contenido.ieprocedencia.documentosEscolares.forEach(element => 
            documentosEscolares += "- "+ element.titulo + "\n"
          );
          return documentosEscolares
        }
      },
  },
  filters: {
