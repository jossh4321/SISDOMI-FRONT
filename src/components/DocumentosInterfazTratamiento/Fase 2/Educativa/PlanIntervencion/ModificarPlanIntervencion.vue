<template>
  <v-card>
    <v-card-title class="justify-center"
      >Modificar Plan de Intervención Educativo</v-card-title
    >
...
export default {
  ...
  async created() {
    ...
    await this.updatePlanIntervencion(this.planIid);
  ...
  methods: {
    ...
    async obtenerPlanIId() {
      await axios
        .get("/documento/PlanIntervencionIndividualEducativo/residente/"+this.residenteDocumento.id)
        .then((x) => {
    ...
    async editPlan() {
      ...
      if (this.$v.$invalid) {      
        ...
        let planI = {
          id: this.planI.id,
          historialcontenido: this.planI.historialcontenido,
    ...
    generarPDF(){
        ...
        doc.text("Apellidos y Nombres: " + this.residenteDocumento.apellido +" "+this.residenteDocumento.nombre+'          Edad: '+this.residenteEdad(), 25, 33);
