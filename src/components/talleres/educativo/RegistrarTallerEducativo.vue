...
export default {
  ...
  watch: {
    searchResidente(value) {
      ...
      axios
        .get("/residente/all/fase/1")//NO HAY...TOCA USAR 1 DE PRUEBA
        .then((res) => {
        ...
        for (let index = 0; index < this.listImages.length; index++) {
          ...
          await axios
            ...
            .then((res) => {
              ...
              this.tallerEdu.firma.cargo = this.user.rol.nombre;
            })
            .catch((err) => {
              console.error(err);
            });
  ...
  computed:{
    ...
    fecharealizacionErrors() {
      ...
      !this.$v.tallerEdu.contenido.fecharealizacion.fechaFvalid2 && errors.push("La fecha de realización debe ser antes del día de hoy");
